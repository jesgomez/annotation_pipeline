...
foreach my $print (keys %genes){
    if (exists $final_mrna{$print}) {
        ...
        foreach (keys %{$evm_transcripts{$print}}) {
            my @line = split /\s+/, $evm_transcripts{$print}{$_};
            if ($line[8] =~ m/ID=([^;]+)/) {$mRNA = $1;}
            $evm_transcripts{$print}{$_} =~ s/ID=$mRNA;/ID=$mRNA.evm;/;
            $evm_exons{$_} =~ s/Parent=$mRNA/Parent=$mRNA.evm/g;
            $evm_exons{$_} =~ s/ID=cds.$mRNA/ID=cds.$mRNA.evm/g;

...
while (<IN>) {
  ...
  if ($gtf[2] eq "gene") {
    ...
    # 	$GENE=$1;
    # 	push(@{$cluster_genes{$cluster}},$GENE);
	...
	$tx{$txid}->{partial}=1;
      ...
      if (exists($starts{$gtf[0]."-".$gtf[3]})){
	 if ($cluster ne $gene_cluster{$starts{$gtf[0]."-".$gtf[3]}}){
	...
	my $previous_cluster = $gene_cluster{$starts{$gtf[0]."-".$gtf[3]}};
	...
	$starts{$gtf[0]."-".$gtf[3]}=$tx{$txid}->{gene};
      }
      if (exists($ends{$gtf[0]."-".$gtf[4]})){
	my $previous_cluster = $gene_cluster{$ends{$gtf[0]."-".$gtf[4]}};
       	if (exists $cluster_genes{$previous_cluster}){
	  foreach my $g (@{$cluster_genes{$previous_cluster}}){
	    if ($gene_cluster{$g} eq $previous_cluster){
	      $gene_cluster{$g}=$cluster;
...
foreach my $transcript (@sorted_transcripts) {
  if (!(exists($transcript->{cds}))){print STDERR $transcript->{id}," has no CDS\n"; next;}
  my $canon = 0;
  ...
  #print STDERR "$prot\n";
  #print STDERR "$tid\n" if !$prot;
  my $gene_id = $gene_cluster{$transcript->{gene}};
  ...
  if (!exists $seen_genes{$gene_id}) {
    ...
    # $clustergff{$gene_id}->[1]=$src;
    # $clustergff{$gene_id}->[2]='gene';
    # $clustergff{$gene_id}->[3]=$cluster_start{$gene_id};
    # $clustergff{$gene_id}->[4]=$cluster_end{$gene_id};
    # $clustergff{$gene_id}->[8]=~s/ID=[^;]+/ID=$newgenename/;
    # $clustergff{$gene_id}->[8]=~s/;Name=[^;]+//;
...
# need to match up protein length with cds length. adjust first exon length by phase or remainder then keep summing exon lengths.
 ...
 # if ($strand eq '+') {
    ...
    $first = 1;
    foreach my $gffrec (sort gffsort @{$transcript->{cds}}) {
    #  my $exonlen = $gffrec->[4]-$gffrec->[3]+1;
     # if ($first){
#	$pstart=1;
#	$exonlen = $gffrec->[4]-$gffrec->[3]+1-$phase;
#	$sum = $gffrec->[7];
#	 #print STDERR (join("\t",@{$gffrec}),"\n");
#	$first = 0;
 #     }else{
#	$exonlen = $gffrec->[4]-$gffrec->[3]+1;
#	$pstart = int(($sum + 1 + 2)/3);
 #     }
  #    $sum+=$exonlen;
   #   $cdscount++;
   #   if ( $cdscount == $numcds){

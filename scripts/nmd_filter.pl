...
my %geneprinted=();
my $tid='';
print STDERR "Filtering out likely NMD targets. Transcripts must be grouped by gene. Gene record first followed by transcript, exons, cds and followed by ###, even last line.\n";
while (<>) {
  ...
  if ( m/^###/) {
	...
	if($nmd){
	  chomp $transcript{$t}->{tgff};$transcript{$t}->{tgff}.=";nmd=true\n";
	}
	print  $transcript{$t}->{tgff};
	print  $transcript{$t}->{gff};
      } else {
	print STDERR "$t subject to NMD\n";
      }
      delete $transcript{$t};
    }
    print "###\n";
